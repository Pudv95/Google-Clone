...
class MySearchBar extends StatelessWidget {
  ...
  final String tag;
  MySearchBar({super.key, required this.searchController,required this.tag});

  List<SearchModel> searchResults = [];
  int start = 0;

  Future<void> getSearchData(context,String query,int start,List<SearchModel> searchResults, bool searching ) async {
    var request = http.Request('GET', Uri.parse(
        'https://www.googleapis.com/customsearch/v1?key=AIzaSyBxMwLzAhJ58R1kIt7jNeomXEm_2Exsp_g&q=$query&cx=5311d6e53f2da48e6&start=$start'));

    http.StreamedResponse response = await request.send();

    if (response.statusCode == 200) {
      Map<String, dynamic> response1 = jsonDecode(
          await response.stream.bytesToString());
      searchResults += (response1['items'] as List)
    ...
    return Hero(
      ...
      child:TextField(
        ...
        onSubmitted: (query) async {
          getSearchData(context,query,start,searchResults,true);
        },
        style: const TextStyle(color: Colors.white),
        decoration:  InputDecoration(
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(50),borderSide: const BorderSide(color: Colors.white)),
          prefixIcon: Padding(
